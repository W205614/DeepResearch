...
企业版 Compose 使用 `keycloak-data` Docker 卷保存注册用户和管理台改动。首次切换到这个版本前，如你正在运行旧版、且需要保留旧容器里临时创建的本地账号，请先在 Keycloak 管理台导出 realm，或重新注册测试账号；不要执行 `docker compose down -v`，它会清空所有命名卷。
