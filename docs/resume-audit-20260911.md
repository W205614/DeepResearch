...
- **路由隔离与质量门禁：** 通过 LLM 语义路由区分普通对话、上下文追问和外部事实研究，支持 Quick/Deep 手动模式；将档案偏好按账号私有保存，会话与报告语义记忆按工作空间及 Thread 约束，在最终报告输出前执行逐结论引用、主体、否定和日期范围核验。离线路由回归 **7/7**；固定证据真实模型评测 **29/30 通过**，引用支持率 **99.66%**、应答覆盖率 **100%**，关键否定题失败触发整体门禁不通过。
