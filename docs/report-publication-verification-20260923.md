...
| Java Maven | 修正后 27/27，通过；其中 21 项 PostgreSQL Testcontainers | 新增部分结果批准说明、未回答问题冻结和工作空间最后管理员保护；原有越权、自审、重复/并发审核、快照冻结和旧接口权限仍通过 |
| Python | 237 通过，4 个条件性跳过；Ruff 通过 | Windows 系统临时目录权限异常，改用仓库 `.cache` 的 pytest 临时目录后全量通过 |
| 前端 | 12/12 单测，类型检查与生产构建通过 | 不替代浏览器验证 |
| 真实浏览器 OIDC | 最新企业套件 6 通过、4 项条件性跳过；此前付费实时研究另行逐项通过 | 两个临时账号通过网页成员设置、部分结果提交、带批准说明的审核、冻结证据读取、资料变更、撤回和审计；跳过项不计入最新通过数 |
