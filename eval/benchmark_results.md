...
本次在提交 `e5310e3` 上执行两轮真实嵌入评测，14 份资料、16 个标注问题，全部检索质量指标两轮一致。Recall@5 为单向量 **0.9688**、混合 **1.0000**；nDCG@5 为 **0.9446 → 0.9676**。

| 本次时延 | 第一轮 | 第二轮 |
| --- | ---: | ---: |
| 单向量平均冷查询 | 1258.44 ms | 981.41 ms |
| 混合平均冷查询 | 1235.77 ms | 1177.63 ms |
| 混合平均热查询 | 131.32 ms | 105.62 ms |
| 热查询缓存命中率 | 100% | 100% |

时延会受本机并行验证负载及远端嵌入服务影响，不由两轮结果推断稳定性能提升。最新脱敏数据见 [audit-20260911.json](audit-20260911.json)，整体核对见 [本次审计](../docs/resume-audit-20260911.md)。以下 2026-09-07 数据保留为历史，不作为当前时延。
