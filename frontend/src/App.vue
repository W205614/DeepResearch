...
import { ArrowUp, ArrowUpRight, BookOpen, Brain, Check, ChevronRight, CircleHelp, Compass, Download, FileText, FolderOpen, Globe2, Layers3, LoaderCircle, Menu, MessageSquare, Plus, RefreshCw, Search, Settings2, ShieldCheck, Sparkles, Square, Trash2, X } from 'lucide-vue-next'
...
const userId = ref(localStorage.getItem('dr-user') || 'local-user'), threadSwitchId = ref(localStorage.getItem('dr-thread') || '')
...
async function download(){if(!selected.value)return;await safely(async()=>{const response=await fetch(`/api/research/runs/${selected.value!.id}/report`,{headers:headers()});if(!response.ok)throw new Error('下载失败');const url=URL.createObjectURL(await response.blob());const a=document.createElement('a');a.href=url;a.download=`research-${selected.value!.id.slice(0,8)}.md`;a.click();URL.revokeObjectURL(url)})}
...
onMounted(async()=>{
  await safely(async()=>{await refreshThreads();await refreshStatus();if(threadId.value&&threads.value.some(t=>t.id===threadId.value))await openThread(threadId.value);else newResearch()})
...
onUnmounted(()=>{controller?.abort();clearInterval(poll)})
</script>
...
<template>
  <div class="workspace">
    ...
    <main>
      <header class="topbar"><div class="breadcrumb"><button class="mobile-menu icon-button" aria-label="打开导航" @click="navOpen=true"><Menu :size="20"/></button><span>工作空间</span><ChevronRight :size="14"/><strong>{{viewTitle}}</strong></div><div class="top-status"><span :class="['status-dot',{'amber':!status||status.missing?.length}]"></span>{{status?.mode==='demo'?'测试模式':status?.missing?.length?'部分服务待配置':'已连接'}}<span class="divider"></span><span>本地部署</span></div></header>
      ...
      <section v-if="view==='research'" class="research-view" :class="{hasRun:selected}">
        ...
        <template v-else>
          <div class="run-header"><div><span class="eyebrow">RESEARCH SESSION</span><h2>连续研究会话</h2><p v-if="conversationHistory.length" class="session-note">已保留 {{conversationHistory.length}} 条历史研究；继续提问会沿用当前会话。</p></div><button v-if="threadId" class="text-button" @click="newTitle=currentTitle;rename=true">重命名会话</button></div>
          ...
          <section v-if="conversationHistory.length" class="conversation-history" aria-label="本会话的历史研究">
            <article v-for="run in conversationHistory" :key="run.id" class="conversation-turn">
              ...
              <div class="conversation-response"><div class="conversation-response-heading"><FileText :size="15"/><strong>研究结果</strong><span>{{stateLabel(run.status)}}</span></div><div v-if="run.report" class="conversation-markdown markdown" v-html="renderReport(run.report)"></div><p v-else class="muted">这次研究尚未生成可展示的报告。</p></div>
            </article>
          </section>
          ...
          <div class="run-layout"><div class="report-column">
            ...
            <article v-if="selected.report" class="report-card"><div class="report-toolbar"><span><FileText :size="16"/>研究报告</span><div><button v-if="selected.status==='completed'" class="text-button" @click="saveRunMemory"><Brain :size="15"/>保存为语义记忆</button><button class="text-button" @click="download"><Download :size="15"/>导出 Markdown</button></div></div><div class="markdown" v-html="reportHtml" @click="inspectCitation"></div><div class="report-metrics"><span>引用检查 {{selected.validation.supported_claims||0}} / {{selected.validation.checked_claims||0}} 条结论</span><span>来源 {{selected.validation.evidence_metrics?.source_count||0}} · 域名 {{selected.validation.evidence_metrics?.unique_web_domains||0}}</span><span>模型调用 {{selected.usage.llm_calls||0}} 次</span><span>搜索 {{selected.usage.search_calls||0}} 次</span></div></article>
