...
const reasonLabel=(key:string)=>({invalid_records:'部分检索字段无效，已剔除',vector_unavailable:'向量检索不可用，已使用关键词检索',ok:'检索完成',empty:'部分检索未找到结果',error:'检索服务发生故障',budget_exhausted:'本轮研究已达到检索上限',search_limit_reached:'本轮研究已达到检索上限，可继续研究',insufficient_evidence:'证据不足，请补充资料或调整问题',image_unreadable:'请上传清晰图片'}[key]||key)
...
async function send() {
  ...
  busy.value=true
  await safely(async()=>{
    const payload={topic:topic.value.trim(),mode:mode.value,thread_id:threadId.value||null,attachment_ids:[...attachmentIds.value]}
...
onMounted(async()=>{
  ...
  poll=setInterval(()=>{if(view.value==='settings'&&authenticated.value&&workspace.value?.role==='admin')void safely(async()=>{deadLetters.value=await api(`/api/workspaces/${workspace.value.id}/dead-letters`)});if(view.value==='documents'&&documents.value.some(d=>d.status==='indexing'))void safely(async()=>{documents.value=await api('/api/documents')});if(selected.value&&!terminal(selected.value.status))void refreshRun(selected.value.id,generation).catch(()=>{})},2500)
})
      ...
      <section v-if="view==='research'" class="research-view" :class="{hasRun:selected}">
        <template v-if="!selected">
          ...
          <div class="start-area">
            <div class="composer" @paste="imagePicker?.paste($event)" @dragover.prevent @drop="imagePicker?.drop($event)"><ImageAttachments ref="imagePicker" v-model="attachmentIds" :disabled="busy||active" @busy="attachmentBusy=$event"/><textarea v-model="topic" aria-label="研究主题" maxlength="4000" placeholder="你想研究什么？试着描述主题、时间范围和你关心的问题…" @keydown.enter.exact.prevent="send"></textarea><div class="composer-footer"><div class="mode-picker"><Sparkles :size="15"/><select v-model="mode" aria-label="研究模式"><option value="auto">自动选择</option><option value="deep">深度研究</option><option value="quick">快速问答</option></select></div><span class="composer-hint">Enter 发送 · Shift + Enter 换行</span><button class="send-button" aria-label="开始研究" :disabled="busy||attachmentBusy||(!topic.trim()&&!attachmentIds.length)" @click="send"><LoaderCircle v-if="busy" class="spin" :size="20"/><ArrowUp v-else :size="21"/></button></div></div>
