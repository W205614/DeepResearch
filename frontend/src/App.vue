...
const reviewSubmissionReason = computed(()=>{
  ...
  if(run.validation?.quality!=='complete')return `当前结果为“${qualityLabel(run.validation?.quality||'unknown')}”，未达到送审门槛；请完善资料后发起新的研究任务。`
  ...
  if(checked<1||supported!==checked)return '引用核验尚未全部通过，不能提交审核。'
  return null
})
