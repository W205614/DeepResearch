...
最近一次固定证据真实模型完整复测（2026-09-12，本次流程优化之前）：**30/30 全部通过**，引用支持率 **100%（297/297 条）**、应答覆盖率 **100%**，**固定研究质量门禁通过**，见[完整复测记录](docs/research-full-20260912.md)及[脱敏结果](eval/research-full-20260912.json)。其他指标仍来自 2026-09-11 整体审计：后端 108 项、真实组件 4 项、前端 4 项通过；业务负载 **4576 请求、386/386 任务完成**，Worker 恢复 **53.50 秒**，见[审计与四条项目表述](docs/resume-audit-20260911.md)。旧轮次 29/30 与门禁失败记录保留，下方注明日期的验收记录为历史结果。
...
深度研究默认每条查询取得最多 8 个结果，在全部查询间轮转选择最多 24 个不同网页候选，再读取可访问正文和筛选证据。可用 `.env` 中的 `WEB_RESULTS_PER_QUERY`、`MAX_WEB_CANDIDATES` 调整覆盖范围；提高它们会增加时延、模型上下文和搜索调用成本。工作空间仅限制并发研究数，不设每日查询或 Token 配额；单轮研究保留检索次数、轮次和时限保护；Token 使用会被记录用于成本观测，但不会作为“今日 Token 预算”拒绝用户请求。它是多源抽样与可核查综合，不是对整个互联网的穷尽抓取。
