...
深度研究默认每条查询取得最多 8 个结果，在全部查询间轮转选择最多 24 个不同网页候选，再读取可访问正文和筛选证据。可用 `.env` 中的 `WEB_RESULTS_PER_QUERY`、`MAX_WEB_CANDIDATES` 调整覆盖范围；提高它们会增加时延、模型上下文和搜索调用成本。不设每日查询或 Token 配额，但每项任务受累计时限、外部调用次数与保守 Token 预占预算约束；达到边界后停止调用，恢复不重置预算。实际用量与保守预占分别理解，不能把预占量当作账单。它是多源抽样与可核查综合，不是对整个互联网的穷尽抓取。
