...
最新整体复测（2026-09-11）：后端 108 项、真实组件 4 项、前端 4 项通过；固定研究评测 **29/30**，引用支持率 **99.66%**、应答覆盖率 **100%**，因关键题 `negation-2` 失败，**整体质量门禁不通过**。本次业务负载为 **4576 请求、386/386 任务完成**，Worker 恢复 **53.50 秒**。当前指标以[本次审计与四条项目表述](docs/resume-audit-20260911.md)及[脱敏结果](eval/audit-20260911.json)为准；下方注明日期的验收记录为历史结果。
...
`eval/research_quality_cases.json` 的 30 题固定证据完整研究评测实际执行研究图，图外评分；门槛为关键题全部通过、整体通过率至少 85%、引用支持率至少 95%、应回答问题覆盖率至少 85%。2026-09-11 最新复测 **29/30 通过**，引用支持率 **99.66%（294/295 条）**、覆盖率 **100%**，关键题 `negation-2` 失败，因此整体门禁不通过。此前 30/30 及其他失败记录保留为历史，不能覆盖本轮结果。这是小规模合成题集，不是通用准确率；实时联网验收另行统计。
```powershell
