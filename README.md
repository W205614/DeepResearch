...
迁移 `0012_report_publication` 增加 Java 独占写入的审核快照。任务作者可提交已完成、有已核验引用的 `complete` 或 `partial` 报告；低质量、未核验及作者不明的历史任务会在报告页显示不能提交的原因。`partial` 表示部分问题仍未回答或部分结论未获支持：它可以进入人工审核，但列表和正式详情必须标示“部分结果”，管理员批准时必须写明发布范围及缺口，不能将其当作完整回答。另一位工作空间管理员审核；在“工作台设置 → 工作空间成员”中，管理员可输入对方在设置页复制的账号 ID，添加为管理员，双方无需调用接口。批准后，工作空间成员从独立的“正式报告”列表读取冻结的正文、证据、位置、文档版本和内容哈希；驳回及撤回必须填写原因，撤回会立即停止成员访问。草稿任务、会话导出、SSE 和图片附件仅作者及管理员可见。含审核记录的会话或研究数据不能直接清理，管理员须先撤回并显式清理审核记录，操作进入审计日志。原始资料热更新或删除后，快照仍保留，但页面会标示原文是否仍可访问。生成与检索仍由 Python Agent 负责，发布状态、权限和审计由 Java 负责。详见 [职责与数据契约](docs/java-python-ownership.md)及[本轮验收记录](docs/report-publication-verification-20260923.md)。

本轮在默认 Docker Compose 重建 Java 与 Web 服务，`0012_report_publication` 迁移保持有效；两个真实 OIDC 账号通过网页成员设置完成“作者提交部分结果 → 异人管理员填写批准说明 → 成员读取冻结快照 → 管理员撤回”。最新 Java 27/27（其中 PostgreSQL Testcontainers 21 项）、前端单测 12/12 和构建均通过；浏览器企业套件 6 项通过、4 项条件性跳过。上轮 Python 237 通过/4 跳过，另行运行的三份实时报告图外评分均通过，本轮未重新运行付费实时模型评测。隔离环境中 11 个卷和两份 PostgreSQL 逻辑备份冷恢复成功，观察恢复时间 253.47 秒。固定研究质量门禁上轮为 **29/30、整体通过**，其中 `compare-1` 单题未通过，不能描述为 30/30。
...
- **保护恢复过程**：检查点和结果写入校验执行所有权；恢复与交付重新检查权限、文档版本及原文片段，拒绝旧 Worker 和失效证据写入。
- **RAG 有限降级**：嵌入与向量检索共用独立短时限，超时后可走关键词检索；关键词计算、缓存与语料容量受限。未变更原文的索引重建保留可用旧版，成功后切换；降级不放宽证据标准。
- **运行可观察、数据可恢复**：独立文档 Worker、资源限制、终态与事件事务写入、核心就绪与能力状态分离；增加积压、心跳、供应商阻断、磁盘和服务失联告警。

| 验证层次 | 本轮结果 | 证据边界 |
| --- | --- | --- |
| 后端回归 | 全量 195 通过、4 跳过；随后 RAG/故障组合 37 通过，最终模型/故障组合 36 通过 | 组合测试与全量重叠，不累加；最后调用超时调整由组合回归验证 |
| 前端 | 12 项单测、构建、1 项浏览器模拟接口场景通过 | 不等于真实 OIDC 全流程复测 |
| 真实组件中断 | 单独 4 项集成测试通过；Worker 恢复观察 53.84 秒，Redis 和 Milvus 中断恢复通过 | 模型和搜索是可控替身；单次恢复时间不是 SLA |
| 真实模型 | 固定研究 30/30、独立回答 10/10、新增保留题 3/3 | 合成资料、同模型图外评分，未人工盲评；模型运行后仍有本地保护改动，未保存不可变源码快照 |
| 当前主机部署 | 迁移 `0008_reliability`，API/研究 Worker/文档 Worker 采集正常，9 条告警规则已加载 | 未验证外部告警收件或长期稳定运行 |
| 当前主机冷备恢复 | 11 个数据卷、两套 SQL 恢复、OIDC 配置和两组向量检索通过；观察 129.89 秒 | 隔离恢复副本不执行研究任务；同机恢复不等于异机容灾 |

详细说明见 [可靠性设计与边界](docs/internal-pilot-reliability.md)、[本机运维与人工验收表](docs/local-pilot-operations.md)、[测试汇总](eval/reliability-acceptance-20260913.json)和[部署恢复证据](eval/local-pilot-deployment-20260913.json)。备份、密钥和原始评审正文保留在本机，不提交仓库。

**尚未完成的试用放行项**：机外备份、独立监控、告警收件验证、人工盲评及值守人员安排。下方注明 2026-09-11/12 的性能与质量记录为历史结果，不能用来代表本次改动后的吞吐或稳定性。

后续质量回归：提交 `184b52b` 的远程 Answer Quality Gate 为 **28/30**，比较题出现来源口径扩写，关键注入题出现“结论仅有一条”的无依据排他范围，门禁正确失败。随后在业务链加入保守的枚举范围检查，并约束写作、核验和局部修订；没有降低门槛或改题。修复后本地完整 30 题一次通过，但远程门禁必须按修复提交另行确认。原始失败与验证边界见 [范围误扩张修复记录](docs/answer-quality-scope-fix-20260913.md)。

## 历史改进（2026-09-12）：长任务登录续期、结构化重试与研究提速

- 前端保存并轮换 Refresh Token，请求前在 Access Token 临近过期时续期；API 轮询、SSE 重连与下载共用并发合并的刷新流程。临时续期失败保留凭证，刷新令牌失效或刷新后仍返回 401 时退出登录；重新登录后可恢复原会话。
- 结构化结果校验失败时，重试反馈包含字段路径与长度上限，区分 schema 的 `too_long` 与输出 token 截断 `output_truncated`，不回显原始模型文本。writer 增加章节与结论长度约束，最多重试一次。
- 搜索最多 3 路并发、网页读取最多 6 路并发；单轮检索上限仍由数据库原子控制。核验原文按来源 ID 去重传输，每条结论仅使用其引用的来源；分析输入完全一致才复用检查点结果。
- 报告修订只返回失败结论的替换或删除，保留已通过内容，随后再次核验，避免整篇重写导致结论膨胀和输出超限。

同题深度模式真实运行（DeepSeek v4 Flash + 博查，含 OIDC、Redis Worker）：

| 指标 | 原始运行 | 并发与去重初版 | 加入局部修订的最终版 |
| --- | ---: | ---: | ---: |
| 总耗时 | 214.016 秒 | 218.047 秒 | 139.344 秒 |
| 检索轮数（含首轮） | 3 | 3 | 2 |
| 模型调用 / 搜索调用 | 13 / 12 | 14 / 12 | 12 / 8 |
| 输入 tokens | 345,816 | 344,389 | 218,985 |
| 最终保留结论 / 来源 | 13 / 7 | 20 / 13 | 15 / 11 |

最终样本约 **2 分 19 秒**，比原始样本少约 75 秒。**这不是稳定提速 35% 的证明**：各版只有一次真实运行，最终版少补搜一轮，网页缓存、检索结果和模型输出也不同。固定相同结论与原文的核验请求体去重后缩小 **59.23%**，这是字节数变化，不是时延或 token 指标。
...
系统注册了 10 个职责角色，包含协调路由与普通对话，在同一进程、共享当前配置的模型端点运行。每个 Agent 进入受限工具作用域，输入输出通过 `ResearchState` 交接；`agent_handoff` 事件记录发送方、接收方与产物字段名。单条研究路径不会执行全部角色；这也不代表多个独立模型服务的分布式协作。
...
| 研究分析 | 结构化模型 | 带来源的结论与缺口 |
| 补搜决策 | 结构化模型 | 补充查询与下一步决策 |
| 报告撰写 | 结构化模型 | 结构化草稿 |
...
除用户在界面手动选择“快速问答”或“深度研究”外，路由模型会结合当前 Thread 上下文判断回答是否需要网页、本地资料库或最新可核查外部事实。普通交流、对当前会话的追问和用户档案查询会走聊天或本地记忆动作，不进入研究检索；单点外部事实使用快速研究，多维或对比主题使用深度研究。同一 Thread 是一条连续研究会话：页面按时间连续显示提问与报告；内部范围下后端注入最多 `CONVERSATION_TURN_LIMIT` 个历史问题和 `CONVERSATION_RECENT_RUNS` 份最近报告；公开范围不注入私有历史，所有模型请求还受统一上下文预算约束。这两个值默认是 30 和 6，可在 `.env` 调整。会话历史、用户设定及历史研究摘要只用于理解上下文，不能作为本次研究事实来源。每次证据审查会记录来源数量、网页域名数、来源类型和正文/摘要状态，便于后续测量覆盖度与来源多样性。
...
重要的数字、日期、比例、金额和法规结论必须有正文或本地资料支持，使用网页依据时需要两个独立域名，或一个在 `SOURCE_TRUST_OVERRIDES` 中明确配置的可信域名；仅使用内部资料时，结论必须明确限定为所提供资料的口径，并通过原文核验。搜索摘要可以用于背景，但不能单独支撑这类结论。网页正文会安全缓存 `WEB_CACHE_TTL_HOURS` 指定的时长；缓存和重试不会绕过 DNS、跳转和内网地址限制。

深度研究默认每条查询取得最多 8 个结果，在全部查询间轮转选择最多 12 个不同网页候选，再读取可访问正文和筛选证据。可用 `.env` 中的 `WEB_RESULTS_PER_QUERY`、`MAX_WEB_CANDIDATES` 调整覆盖范围；提高它们会增加时延、模型上下文和搜索调用成本。每条搜索和正文抓取还分别受 `WEB_SEARCH_TIMEOUT_SECONDS`（默认 20 秒）和 `WEB_FETCH_TIMEOUT_SECONDS`（默认 8 秒）的单次上限保护，慢站点只会形成检索局限，不会无限拖住整轮研究。不设每日查询或 Token 配额，但每项任务受累计时限、外部调用次数与保守 Token 预占预算约束；达到边界后停止调用，恢复不重置预算。实际用量与保守预占分别理解，不能把预占量当作账单。它是多源抽样与可核查综合，不是对整个互联网的穷尽抓取。

同一 Worker 内，搜索与网页读取分别由 `WEB_SEARCH_CONCURRENCY`（默认 3）和 `WEB_FETCH_CONCURRENCY`（默认 6）限制并发，单轮搜索次数仍由数据库原子计数控制。引用核验按结论分批，只传该批声明的来源并保留引用原文完整内容。补搜后，只有分析输入完全相同时才复用该任务检查点中的分析结果；证据、问题或冲突改变会重新分析。可将搜索并发设为 1、网页并发设为 3 恢复原有请求并发水平。核验不通过时只请求对应结论的替换或删除，已通过内容原样保留；修订按失败结论分批，只携带该批引用的相关原文窗口和分析事实，所有替换再次使用完整引用原文核验，未知引用和重复修订索引不会进入最终报告，避免整篇重写增加结论数量和输出超限。
...
`eval/research_quality_cases.json` 的 30 题固定证据完整研究评测实际执行研究图，图外评分；门槛为关键题全部通过、整体通过率至少 85%、引用支持率至少 95%、应回答问题覆盖率至少 85%。2026-09-12 修复术语边界后，保持原题与评分门槛，单次完整复测 **30/30 通过**，引用支持率 **100%（297/297 条）**、覆盖率 **100%**，全部关键题通过，固定研究质量门禁通过，见[本次记录](docs/research-full-20260912.md)。2026-09-11 的 29/30 及关键题失败结果仍保留为历史。这是小规模合成题集，不是通用准确率；图外评分使用同一模型，不是人工盲评；实时联网验收另行统计。
```powershell
...
新增迁移 `0004_consistency` 保留已有数据。资料版本阻止删除后迟到索引复活；请求幂等先于新任务额度校验；UTC 日搜索台账仅用于统计，单任务检索次数原子预占，失败调用及备用搜索分别计数；Worker 尝试版本约束心跳与终态，持久化任务对账补偿 Redis 投递失败。
...
输入框支持最多 4 张图片，自动区分直接看图和需要检索的研究；图片按任务保存，不自动进入共享资料库。检索空结果、服务故障和本轮检索达到上限会分别提示，证据不足时不生成无依据的事实报告。详见 [图片输入与 Grafana 观测说明](docs/multimodal.md)。
