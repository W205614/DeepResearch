...
本轮在默认 Docker Compose 重建 Java 与 Web 服务，`0012_report_publication` 迁移保持有效；两个真实 OIDC 账号通过网页完成成员添加、作者提交部分结果、异人管理员审核、成员读取冻结快照、撤回和成员移除。移除后原线程报告访问返回 403，审计记录仍在。最新 Java 28/28（其中 PostgreSQL Testcontainers 22 项）、前端单测 12/12 和构建均通过；浏览器企业套件 6 项通过、4 项条件性跳过。上轮 Python 237 通过/4 跳过，另行运行的三份实时报告图外评分均通过，本轮未重新运行付费实时模型评测。隔离环境中 11 个卷和两份 PostgreSQL 逻辑备份冷恢复成功，观察恢复时间 253.47 秒。固定研究质量门禁上轮为 **29/30、整体通过**，其中 `compare-1` 单题未通过，不能描述为 30/30。
