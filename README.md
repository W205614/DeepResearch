...
同一 Worker 内，搜索与网页读取分别由 `WEB_SEARCH_CONCURRENCY`（默认 3）和 `WEB_FETCH_CONCURRENCY`（默认 6）限制并发，单轮搜索次数仍由数据库原子计数控制。引用核验按结论分批，并只传该批声明的来源；当单批原文仍过长时，按结论选择相关原文窗口并明确记录截取状态。补搜后，只有分析输入完全相同时才复用该任务检查点中的分析结果；证据、问题或冲突改变会重新分析。可将搜索并发设为 1、网页并发设为 3 恢复原有请求并发水平。核验不通过时只请求对应结论的替换或删除，已通过内容原样保留；修订同样按失败结论分批，只携带该批引用和相关分析事实，所有替换再次核验，未知引用和重复修订索引不会进入最终报告，避免整篇重写增加结论数量和输出超限。
